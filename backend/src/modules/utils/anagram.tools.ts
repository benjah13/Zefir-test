...
export function writeAnagramsFile(nbWords: any, filename: any, maxLength: number, minLength: number, callback: () => void) {
  ...
  function* buildAnagrams(input: string) {
    function swap(chars: { [x: string]: any }, i: number, j: number) {
      const tmp = chars[i];
      chars[i] = chars[j];
      chars[j] = tmp;
    }
  ...
  function* generate(words: string | any[] | undefined) {
    if (words)
  ...
  class AddEOL extends Transform {
    _transform(chunk: { toString: (arg0: string) => any }, enc: any, done: () => void) {
  ...
  genStream
    ...
    .pipe(writeStream)
    .on('finish', function () {
      console.log(`Finished writing to file ${filename} in ${Date.now() - start} miliseconds`);
      callback();
    });
}
